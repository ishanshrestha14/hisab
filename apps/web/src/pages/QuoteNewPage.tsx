...
import { createQuoteSchema, type CreateQuoteInput, type Currency } from "@hisab/shared";
...
function LineItemsTable({
  ...
  control: ReturnType<typeof useForm<CreateQuoteInput>>["control"];
  register: ReturnType<typeof useForm<CreateQuoteInput>>["register"];
  errors: ReturnType<typeof useForm<CreateQuoteInput>>["formState"]["errors"];
}) {
...
export default function QuoteNewPage() {
  ...
  const mutation = useMutation({
    mutationFn: (data: CreateQuoteInput) => api.post("/api/quotes", data),
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-foreground">New Quote</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Create an estimate to send to a client
        </p>
      </div>
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <select
                  {...register("clientId")}
                  onChange={(e) => { register("clientId").onChange(e); handleClientChange(e); }}
                  className={inputClass}
                >
              ...
              <div>
                ...
                {errors.issueDate && <p className={errorClass}>{errors.issueDate.message as string}</p>}
              </div>

              <div>
                <label className={labelClass}>Expiry Date *</label>
                <input {...register("expiryDate")} type="date" className={inputClass} />
                {errors.expiryDate && <p className={errorClass}>{errors.expiryDate.message as string}</p>}
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div>
                ...
                <textarea
                  {...register("notes")}
                  rows={3}
                  placeholder="Scope of work, validity conditions, etc."
