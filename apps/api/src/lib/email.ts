...
export async function sendOverdueReminderEmail(opts: {
  ...
  return resend.emails.send({
    from: FROM,
    to: opts.to,
    subject: `Overdue: ${opts.invoiceNumber} — ${amount} was due ${due}`,
    html: `
<!DOCTYPE html>
<html>
  ...
  <div style="max-width: 540px; margin: 0 auto; background: #ffffff; border-radius: 8px; border: 1px solid #e5e7eb; overflow: hidden;">
    ...
    <div style="padding: 32px;">
      ...
      <p style="margin: 0 0 8px; font-size: 15px; color: #374151;">
        Invoice <strong>${opts.invoiceNumber}</strong> sent to <strong>${opts.clientName}</strong> is now overdue.
      </p>
      <p style="margin: 0 0 24px; font-size: 15px; color: #374151;">
        <strong>${amount}</strong> was due on <strong>${due}</strong>.
      </p>
      <a href="${opts.portalUrl}" style="display: inline-block; background: #dc2626; color: #ffffff; font-size: 15px; font-weight: 600; text-decoration: none; padding: 12px 28px; border-radius: 6px;">
        View Invoice
      </a>
      <p style="margin: 32px 0 0; font-size: 13px; color: #9ca3af;">
        Share this link with your client to collect payment:<br/>
